...
export default function Zones() {
  return (
    <section id="zonas" className="section" style={{ background: 'var(--stone)' }}>
      <div className="wrap">
        <div className="zones-head" style={{ display: 'grid', gridTemplateColumns: '180px 1fr', gap: 64, marginBottom: 64, alignItems: 'start' }}>
          <div>
          ...
          <div>
            <h2 className="headline" style={{ fontSize: 'clamp(32px, 4.4vw, 60px)', lineHeight: 1.08, margin: '0 0 24px', maxWidth: 800 }}>
              El norte de Madrid, <span style={{ fontStyle: 'italic', fontFamily: "'Fraunces', serif", fontWeight: 300, color: 'var(--accent)' }}>de cerca</span>.
            </h2>
            <p style={{ fontSize: 18, lineHeight: 1.7, color: 'var(--muted)', margin: 0, maxWidth: 620 }}>
              Trabajo principalmente en el corredor de la A-1 — las urbanizaciones privadas del norte de Madrid que conozco desde dentro.
            </p>
          </div>
        </div>

        <div className="zones-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 32 }}>
          {ZONES.map((z) => (
            <article key={z.name} style={{ background: 'var(--cream)', border: '1px solid var(--line)', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
              ...
              <div style={{ padding: '28px 28px 32px' }}>
                <div className="headline" style={{ fontSize: 24, marginBottom: 4 }}>{z.name}</div>
                <div className="eyebrow" style={{ color: 'var(--accent)', marginBottom: 16 }}>{z.place}</div>
                <p style={{ fontSize: 15, lineHeight: 1.65, color: 'var(--muted)', margin: '0 0 20px' }}>{z.desc}</p>
